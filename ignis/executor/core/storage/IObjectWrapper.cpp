...
class IReadWrapperIterator : public ICoreReadIterator<IObject::Any>{
public:
    IReadWrapperIterator(const std::shared_ptr<ICoreReadIterator<IObject::Any>> &to_read,
                         const std::shared_ptr<ICoreWriteIterator<IObject::Any>> &to_write) : to_read(to_read),
                                                                                             to_write(to_write) {}
...
private:
    std::shared_ptr<ICoreReadIterator<IObject::Any>> to_read;
    std::shared_ptr<ICoreWriteIterator<IObject::Any>> to_write;
};

std::shared_ptr<ICoreReadIterator<IObject::Any>> IObjectWrapper::readIterator() {
    return std::make_shared<IReadWrapperIterator>(to_read->readIterator(), to_write->writeIterator());
}

std::shared_ptr<ICoreWriteIterator<IObject::Any>> IObjectWrapper::writeIterator() {
    return to_read->writeIterator();
}

void IObjectWrapper::read(std::shared_ptr<apache::thrift::transport::TTransport> trans) {
    to_read->read(trans);
}

void IObjectWrapper::write(std::shared_ptr<apache::thrift::transport::TTransport> trans, int8_t compression) {
