...
uint32_t IHeaderTransport::read(uint8_t *buf, uint32_t len) {
    if (header.empty()) {
        return trans->read_virt(buf, len);
    }
...
void IHeaderTransport::write(const uint8_t *buf, uint32_t len) {
    trans->write_virt(buf, len);
}
...
void IHeaderTransport::consume(uint32_t len) {
    trans->consume_virt(len);
}
