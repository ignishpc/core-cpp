...
unsigned int RTTInfo::getNumOfBases() {
    if (isVirtalClass()) {
...
const std::type_info &RTTInfo::getBase(int p) {
    if (isVirtalClass()) {
        const abi::__vmi_class_type_info *class_info = (const abi::__vmi_class_type_info *) &this->info;
...
const std::type_info &RTTInfo::getBase() {
    return getBase(0);
}

bool RTTInfo::isPointer() {
    return typeid(abi::__pointer_type_info) == typeid(this->info);
}

const std::type_info &RTTInfo::getClassPointed() {
...
bool RTTInfo::isMemberPointer() {
    return typeid(abi::__pointer_to_member_type_info) == typeid(this->info);
}

const std::type_info &RTTInfo::getClassMemberPointed() {
