...
public:
    ISplitterIterator(shared_ptr<ICoreReadIterator<IObject::Any>> it_read,
            ...
            : mutex_read(mutex_read), mutex_write(mutex_write), it_read(it_read), it_write(it_write), buffer(buffer) {
...
public:
    ...
    IOrderSplitterIterator(size_t id, size_t threads, shared_ptr<ICoreReadIterator<IObject::Any>> it_read,
                           shared_ptr<ICoreWriteIterator<IObject::Any>> it_write, std::mutex &mutex_read,
                           std::mutex &mutex_write, size_t buffer, size_t &write_next,
                           shared_ptr<vector<boost::lockfree::spsc_queue<std::shared_ptr<IObject>>>> &queues)
            : id(id), ISplitterIterator(it_read, it_write, mutex_read, mutex_write, buffer), write_next(write_next),
...
IThreadSplitter::IThreadSplitter(IObject &object_in, IObject &object_out, size_t threads, size_t buffer, bool order) :
    ...
    if (order) {
        ...
        for (size_t i = 0; i < threads; i++) {
            auto it = make_shared<IOrderSplitterIterator>(i, threads, it_read, it_write, mutex_read, mutex_write,
                                                          buffer, write_next, queues);
            iterators.push_back(pair<decltype(it), decltype(it)>(it, it));
        }
    } else {
        for (size_t i = 0; i < threads; i++) {
            auto it = make_shared<ISplitterIterator>(it_read, it_write, mutex_read, mutex_write, buffer);
            iterators.push_back(pair<decltype(it), decltype(it)>(it, it));
        }
    }
}